      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {items.map((item, index) => {
          ...
          return (
            <div 
              key={`${item.name}-${index}`} 
              className={`bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 border border-gray-100 dark:border-gray-700 group flex flex-col ${item.isAvailable === false ? 'grayscale opacity-70' : ''}`}
            >
              <div className="h-48 overflow-hidden relative">
                ...
                {item.isAvailable === false && (
                  <div className="absolute inset-0 bg-black/40 backdrop-blur-[1px] flex items-center justify-center">
                    <span className="bg-red-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-2xl">
                      Sold Out
                    </span>
                  </div>
                )}
              ...
              <div className="p-6 flex-1 flex flex-col">
                ...
                {item.isAvailable !== false ? (
                  ...
                  <div className="mt-auto w-full py-4 bg-gray-100 dark:bg-white/5 text-gray-400 rounded-xl font-black text-xs uppercase tracking-widest text-center">
                    Out of Stock
                  </div>
                )}
              </div>
